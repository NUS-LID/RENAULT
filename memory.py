...
class MultiSegmentTree():
  def __init__(self, size, n):
    self.data = np.array([blank_trans] * size, dtype=Transition_dtype)  # Build structured array
    self.trees = [SegmentTree(size) for _ in range(n)]
    self.index = 0
    self.full = False
    self.size = size

  def update(self, tid, indices, values):
    self.trees[tid].update(indices, values)

  def append(self, tids, data):
    assert len(tids) > 0, "The data should be assigned to at least one tree"
    self.data[self.index] = data  # Store data in underlying data structure
    for tid in tids:
      value = self.trees[tid].max
      self.trees[tid]._update_index(self.index + self.trees[tid].tree_start, value)  # Update tree
      self.trees[tid].max = max(value, self.trees[tid].max)
    self.index = (self.index + 1) % self.size  # Update index
    self.full = self.full or self.index == 0  # Save when capacity reached

  def find(self, tid, values):
    return self.trees[tid].find(values)

  def get(self, data_index):
    return self.data[data_index % self.size]

  def total(self, tid):
    return self.trees[tid].sum_tree[0]
...
class ReplayMemory():
  def __init__(self, args, capacity):
    ...
    n_tree = self.args.n_member if not args.agent == 'REN-J' else 1
    self.transitions = MultiSegmentTree(capacity, n_tree)  # Store transitions in a wrap-around cyclic buffer within a sum tree for querying priorities
  ...
  def append(self, state, action, reward, terminal):
    ...
    if not self.args.agent == 'REN-J':
      while True: # sample tree ids
        tids = []
        for tid in range(self.args.n_member):
          if np.random.rand() <= self.args.buffer_prob:
            tids.append(tid)
        if len(tids) > 0:
          break
    else:
    ...
    self.transitions.append(tids, (self.t, state, action, reward, not terminal))  # Store new transition with maximum priority
  ...
  def _get_samples_from_segments(self, tid, batch_size, p_total):
    ...
    while not valid:
      ...
      probs, idxs, tree_idxs = self.transitions.find(tid, samples)  # Retrieve samples from tree with un-normalised probability
    ...
    # ---
    d_rewards = torch.tensor(np.copy(transitions['reward'][:, self.history - 1]), dtype=torch.float32, device=self.device)
    d_next_states = torch.tensor(all_states[:, 1: 1 + self.history], device=self.device, dtype=torch.float32).div_(255)
    # ---

    return probs, idxs, tree_idxs, states, actions, R, next_states, nonterminals, d_rewards, d_next_states, rewards

  def sample(self, tid, batch_size):
    p_total = self.transitions.total(tid)  # Retrieve sum of all priorities (used to create a normalised probability distribution)
    probs, idxs, tree_idxs, states, actions, returns, next_states, nonterminals, d_rewards, d_next_states, rewards = self._get_samples_from_segments(tid, batch_size, p_total)  # Get batch of valid samples
    probs = probs / p_total  # Calculate normalised probabilities
    capacity = self.capacity if self.transitions.full else self.transitions.index
    weights = (capacity * probs) ** -self.priority_weight  # Compute importance-sampling weights w
    weights = torch.tensor(weights / weights.max(), dtype=torch.float32, device=self.device)  # Normalise by max importance-sampling weight from batch
    return tree_idxs, states, actions, returns, next_states, nonterminals, weights, d_rewards, d_next_states, rewards

  def update_priorities(self, tid, idxs, priorities):
    priorities = np.power(priorities, self.priority_exponent)
    self.transitions.update(tid, idxs, priorities)
